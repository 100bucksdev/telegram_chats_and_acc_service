...
async def add_message(user_id:int, chat_id: int, message: str = Body(...),
                      sender: Literal['client', 'staff', 'ai'] = Body(...),
                      chat_with_user_id:int = Body(...),
                      message_id:int = Body(...),
                      db: AsyncSession = Depends(get_db)) -> MessageRead:
    account_service = TelegramAccountService(db)
    chat_service = ChatService(db)
    ...
    if not account:
        raise HTTPException(
            status_code=404, detail='account_not_found')
    chat = await chat_service.get_chat_by_chat_id(chat_id)
    if not chat:
        chat = await chat_service.create(ChatCreate(chat_id=chat_id, account_id=account.id, chat_with_user_id=chat_with_user_id))

    message = await message_service.create(MessageCreate(message_text=message, chat_id=chat.id, sender=sender, message_id=message_id))
